...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        return attn_output, attn_weights_reshaped, past_key_value, value_states # returning value vectors as well for Kobayashi's scores
...
class Wav2Vec2EncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False, 
        ...
        hidden_states, attn_weights, _, value_states = self.attention(
...
class Wav2Vec2Encoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs = layer(
                        hidden_states, attention_mask=attention_mask, output_attentions=output_attentions, output_norms=output_norms
                    )
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, all_hidden_states, all_self_attentions, all_norms, all_summed_norms] if v is not None)
        return BaseModelOutput(
            last_hidden_state=hidden_states,
            hidden_states=all_hidden_states,
            attentions=all_self_attentions,
            norms=all_norms, 
            summed_norms=all_summed_norms
        )
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        return Wav2Vec2BaseModelOutput(
            ...
            summed_norms=encoder_outputs.summed_norms
        )
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutput(
            loss=loss, logits=logits, hidden_states=outputs.hidden_states, attentions=outputs.attentions, norms=outputs.norms, summed_norms=outputs.summed_norms
        )
